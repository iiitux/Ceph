...
* 编译安装还依赖以上的库文件，因为安装了kmod，需要重启系统；
...
* 源代码编译tcmu-runner，安装时需要执行`install_dep.sh`来安装依赖包；
...
```bash
[root@ceph01 ~]# git clone https://github.com/ceph/ceph-iscsi-tools.git
[root@ceph01 ~]# cd ceph-iscsi-tools/
[root@ceph01 ceph-iscsi-tools]# python setup.py install
```

* 在ceph01、ceph02、ceph03节点上编译安装`ceph-iscsi-tools`；
